...
impl IntoResponse for ApiError {
    fn into_response(self) -> Response {
        ...
        (StatusCode::INTERNAL_SERVER_ERROR, format!("conversion failed: {}", self.0)).into_response()
    }
}
...
async fn convert(State(state): State<AppState>, mut multipart: Multipart) -> Result<Response, ApiError> {
    ...
    let result = convert_inner(&state, &mut multipart, &input_dir, &output_dir, &profile_dir).await;
...
async fn convert_inner(
    ...
    while let Some(field) = multipart.next_field().await? {
        ...
        if name == "format" {
            format = field.text().await?.trim().trim_start_matches('.').to_ascii_lowercase();
            if format.is_empty() || !format.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-') {
    ...
    if !output.status.success() {
        ...
        return Err(anyhow!("LibreOffice exited with {}: {} {}", output.status, stderr.trim(), stdout.trim()).into());
    }
    ...
    response.headers_mut().insert(
        ...
        HeaderValue::from_str(&format!("attachment; filename=\"{}\"", download_name.replace('"', "_")))?,
    );
    Ok(response)
}
...
fn sanitize_filename(name: &str) -> String {
    ...
    let safe: String = base.chars().map(|c| if c.is_ascii_alphanumeric() || matches!(c, '.' | '_' | '-') { c } else { '_' }).collect();
    if safe.is_empty() { "input".into() } else { safe }
}
